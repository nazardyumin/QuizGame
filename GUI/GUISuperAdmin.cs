...
namespace QuizGame.GUI
{
    public class GUISuperAdmin : GUIAdmin
    {
        ...
        private (bool keep_on, bool logout, bool back, SomeAction action) MainMenuWindow()  //надо переопределить
        {
            ...
            win.Add(hello);
            var create_quiz = new Button("Create Admin");
            ...
            var edit_quiz = new Button("Create SuperAdmin");
            edit_quiz.X = Pos.Center();
            edit_quiz.Y = Pos.Top(create_quiz) + 2;
            edit_quiz.Clicked += () =>
            {
                action = CreateSuperAdminWindow;
                top.Running = false;
            };
        ...
        private (bool keep_on, bool logout, bool back, SomeAction action) CreateSuperAdminWindow()
        {
            bool logout = false;
            bool keep_on = true;
            bool back = false;
        ...
        public (bool keep_on, bool logout) SuperAdminMenu()
        {
            bool keep_on = true;
            bool logout = false;
            do
